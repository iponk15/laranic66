...
{
    private $url       = 'menus';
    private $titlehead = 'Data Menu';
    private $table     = 'mrt_menus';
    private $route     = 'menus';
		   ...
		   try {
		     ...
		     switch($_GET['operation']) {
		       case 'get_node':
		         ...
		         $data = DB::table($this->table)->selectRaw("menu_id AS id, menu_link AS name, menu_nama AS text, (CASE WHEN menu_parent IS NULL THEN '0' ELSE menu_parent END) AS parent_id, menu_icon AS icon, menu_link")->orderBy('menu_order', 'asc')->get();
		       ...
		       case 'create_node':
		         ...
		         DB::table($this->table)->insert(['menu_nama' => $nodeText, 'menu_parent' => $node, 'menu_order' => $_GET['position'], 'created_by' => Auth::id(), 'created_at' => date('Y-m-d H:i:s'), 'menu_ip' => $request->ip()]);
		       ...
		       case 'rename_node':
		         ...
		         //print_R($_GET);
		         $nodeText = isset($_GET['text']) && $_GET['text'] !== '' ? $_GET['text'] : '';
		         DB::table($this->table)->where('menu_id', $node)->update(['menu_nama' => $nodeText]);
		         break;
		       case 'delete_node':
		         $node = isset($_GET['id']) && $_GET['id'] !== '#' ? (int)$_GET['id'] : 0;
		         DB::table($this->table)->where('menu_id', $node)->delete();
		         DB::table($this->table)->where('menu_parent', $node)->delete();
		         break;
		       case 'move_node':
			       	DB::beginTransaction();
					...
					try {
						...
						$update = DB::table($this->table)->where('menu_id', $node)->update(['menu_parent' => $parent, 'menu_order' => $position]);
						/*BEGIN GET MENU YANG SUDAH DI UPDATE*/

						/*BEGIN GET DATA MENU & BUILD ARRAY AFTER UPDATE MENU ORDER NODE YANG DI DRAG*/
						$NewMenu = DB::table($this->table)->selectRaw('menu_id, menu_order, menu_nama')->where('menu_parent', $parent)->orderBy('menu_order', 'asc')->get();
						...
						if (isset($NewUpt)) {
							foreach ($NewUpt as $value) {
								DB::table($this->table)->where('menu_id', $value['menu_id'])->update(['menu_order' => $value['menu_order']]);
							}
						}
    ...
    function detail(){
		...
		$data['id']	     = $id;
		$data['route']   = $this->route;
		$data['records'] = DB::table($this->table)->where('menu_id', $id)->get();;
    ...
    function store_parent(Request $req){
        ...
        $countOrder = Mrt_menu::whereNull('menu_parent')->count();

        $data = new Mrt_menu([
            'menu_nama' 		=> $post['menu_nama'],
            'menu_order'		=> $countOrder,
            'created_by'		=> Auth::id(),
            'menu_ip'			=> $req->ip()
        ]);
